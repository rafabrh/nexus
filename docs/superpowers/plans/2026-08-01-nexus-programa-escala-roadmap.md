...
| 2 — Contrato + barramento | 🟢 software completo (2.1+2.2 prod; 2.3 prod; 2.4 em PR) — resta Fase 0 🔒 |
...
### Etapa 2 — Contrato + barramento — 🟢 SOFTWARE COMPLETO (4/4) · resta Fase 0 🔒
...
- 🔵 **Fatia 2.4 — `EvolutionClient` port + 2 adapters** — **ENTREGUE** (plano `2026-08-03-evolution-client-port.md`, subagent-driven, 4 implementers). Port `EvolutionGateway` (superfície extraída) + `EvolutionNodeAdapter` (client atual 1:1) + `EvolutionGoAdapter` (@provisional, lança até Fase 0) + **`EvolutionClient` vira router** (mesmo token; delega por `gatewayFor(instancia)` do snapshot da 2.3 — consumidores INALTERADOS) + `listWithGateway`/`gatewayFor` no snapshot. Auditoria `/hm-engineer`: **0 crítico/alto, 1 médio + 2 baixos, TODOS gated (Fase 0)** → veredito SHIPPA. **404 testes apps/api + 34 shared, lint ok, 0 regressão. PR aberto.** Envio real GO 🔒 (Fase 0). **→ Etapa 2 fecha 4/4 no software 🟢.**

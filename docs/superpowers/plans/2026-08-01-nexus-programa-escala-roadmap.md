...
- 📋 **Fatia 2.3 — Config store por tenant** — **plano escrito** `2026-08-02-tenant-config-store.md` (8 tasks TDD, pronto p/ executar). `tenant_engine_config(instancia, config jsonb, cfg_version)` (migration Drizzle, conferir journal) + `gateway`/`transport` no registry (D7) + write-through Redis (`tenant:cfg:*`) + reconcile boot/periódico + **`InMemoryGatewayConfigStore`** (impl SÍNCRONA que cabeia o seam GO da 2.2: `resolveInstanceId`/`ownerJid`) + seed SQL. Decisão-chave: o seam é sync (normalizer puro) → snapshot em memória hidratado do Postgres. Finding #3 (shape v1-GO) e ativação seguem gate de Fase 0. 100% testável agora.

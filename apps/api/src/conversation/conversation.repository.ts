...
export class ConversationRepository {
  ...
  async buildListItem(instancia: string, jid: string): Promise<ConversationListItem> {
    ...
    const vals = results!.map(([, val]) => val as string | null);

    const [stageRaw, humanCtrl, payment, optout, tagsRaw, contactRaw] = vals;
  ...
  async buildDetail(instancia: string, jid: string): Promise<ConversationDetail | null> {
    ...
    pipeline.llen(RedisKeys.chatHistory(instancia, jid));        // 7

    const results = await pipeline.exec();
    if (!results) return null;

    const vals = results.map(([, val]) => val);
    const [stageRaw, humanCtrl, payment, optout, tagsRaw, contactRaw, notasRaw, messageCount] = vals as [
      string | null,
      string | null,
      string | null,
      string | null,
    ...
    const funnelStage = FunnelStage.fromString(stageRaw);
    const aiState = this.resolveAiState(humanCtrl as string | null);
    const tags: string[] = tagsRaw ? this.safeParseArray(tagsRaw) : [];
